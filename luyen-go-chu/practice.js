...
const GOOGLE_SHEET_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQCQoay5fefKoxYkIIeRuinvpy5uBUzul42A6jSxFUlq4bBvvYnm3y95vn7VFexFaNuhkjWPaLhDUuy/pub?output=tsv';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', (e) => { if (document.activeElement === typingInputEl) { if (['Tab', ' '].includes(e.key)) e.preventDefault(); const key = e.key === ' ' ? ' ' : e.code.startsWith('Key') ? e.key : e.code; highlight(key); } });
    document.addEventListener('keyup', (e) => { const key = e.key === ' ' ? ' ' : e.code.startsWith('Key') ? e.key : e.code; unhighlight(key); });
